...
function ProductImage({ imageKey }: { imageKey: string | null | undefined }) {
  ...
  useEffect(() => {
    if (imageKey) {
      getUrl({ key: imageKey }).then(result => {
...
function App({ signOut, user }: AppProps) {
  ...
  const filteredProducts = products
    .filter(product => 
      product.engword.toLowerCase().includes(searchTerm.toLowerCase()) &&
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        default: return a.engword.localeCompare(b.engword)
      }
    })
        ...
        <Flex justifyContent="space-between" alignItems="center">
          <View>
            <Flex alignItems="center" gap="0.75rem">
              ...
              >
                <Text fontSize="xl" style={{ margin: 0 }}>🏪</Text>
              </View>
              <View>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '2rem' }}>
        ...
        >
          ...
          >
            <Flex alignItems="center" gap="0.5rem">
              ...
              >
                <Text fontSize="xs" style={{ margin: 0, filter: 'brightness(0) invert(1)' }}>🔍</Text>
              </View>
        ...
        {currentUser && (
          <Card marginBottom="1rem" style={{ backgroundColor: '#f0f9ff', border: '1px solid #7dd3fc', borderRadius: '12px' }}>
            <Flex alignItems="center" gap="1rem" padding="1rem">
              <Badge style={{ backgroundColor: '#3b82f6', color: 'white', fontWeight: '600' }}>
                {currentUser.role}
              </Badge>
        ...
        {showUserManagement && canManageUsers && (
          <Card marginBottom="2rem" style={{ backgroundColor: '#fefce8', border: '1px solid #fde047', borderRadius: '12px' }}>
            <View padding="1.5rem">
              <Text fontSize="lg" fontWeight="700" marginBottom="1rem">👥 User Management</Text>
              {allUsers.map(user => (
                <Flex key={user.id} alignItems="center" gap="1rem" marginBottom="1rem" padding="1rem" 
                      style={{ backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                  <View flex="1">
                    <Text fontWeight="600">{user.name}</Text>
                    <Text fontSize="sm" color="#6b7280">{user.email}</Text>
                  </View>
                  <SelectField
                    label="Role"
                    value={user.role}
        ...
        {(lowStockAlert || lowStockProducts.length > 0) && (
          ...
          >
            <Flex alignItems="center" gap="1rem" padding="1rem">
              ...
              >
                <Text fontSize="lg" style={{ margin: 0 }}>⚠️</Text>
              </View>
              <View>
